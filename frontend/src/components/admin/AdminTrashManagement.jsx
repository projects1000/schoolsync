...
const AdminTrashManagement = () => {
    ...
    const executeRestore = async () => {
        ...
        try {
            if (selectedItem.type === 'students') {
                await adminService.restoreStudent(selectedItem.id);
            } else if (selectedItem.type === 'teachers') {
    ...
    const renderTable = (items, type) => {
        if (items.length === 0) {
            return (
                <div className="bg-white rounded-xl shadow-sm p-12 text-center border mt-6">
                    <Trash2 className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                    <h3 className="text-xl font-semibold text-gray-600 mb-2">Trash is Empty</h3>
                    <p className="text-gray-400">No deleted {type} found.</p>
                </div>
            );
        }
                            ...
                            {items.map((item) => (
                                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <Button
                                            onClick={() => handleRestoreClick(item, type)}
    ...
    return (
        <div className="space-y-6">
            ...
            >
                ...
                {/* Tabs */}
                <div className="flex space-x-1 mt-6 bg-gray-100/50 p-1 rounded-lg w-fit">
                    {[
            ...
            >
                {isLoading ? (
                    ...
                    renderTable(deletedItems[activeTab], activeTab)
                )}
            </motion.div>
                        ...
                        >
                            <div className="text-center">
                                ...
                                <h3 className="text-lg font-semibold text-gray-800 mb-2">
                                    Restore {selectedItem?.type.slice(0, -1)}?
                                </h3>
                                <p className="text-gray-500 text-sm mb-6">
                                    Are you sure you want to restore <strong>{selectedItem?.name}</strong>? It will be restored with INACTIVE status.
                                </p>
                            </div>
...
export default AdminTrashManagement;
